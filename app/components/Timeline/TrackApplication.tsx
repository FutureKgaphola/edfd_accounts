                    ...
                    <div className="z-10 -mt-36 scroll-m-8">
                        <Card className="max-w-screen-xl w-full h-fit m-4 p-2 self-center">
                            ...
                            {Applications
                                ...
                                ?.map((item:any) => (
                                    <TimeLineChildComponent key={item.id} id={item.applicationRef} date={item.create_date} Company={item.companyName} status={item.status} category={item.loanDocs} RegNo={item.regNo} body={item.message} stage={item?.stageAt} outcome={item?.outcome} recommendations={item?.recommendation
} />
                                ))}
                            </div>
                        </Card>
