...
const EditDirectors = () => {
    ...
    const dirprop = useSelector((state: RootState) => state.DirectorReducer) as { Director: { fullnames: string; email: string; phone: string } | null; openModal: boolean };
            ...
            <Modal show={dirprop.openModal} onClose={() => dispatch(DirectorAction.SetGlobalDirector({ director: null, openModal: false }))}>
                ...
                <Modal.Body>
                    <form onSubmit={async (e) => {
                        ...
                        const resp = await addDirectorWithDocs({ regNo: regNo, fullnames: directorFullName, email: email, phone: phone,called:'update'  });
